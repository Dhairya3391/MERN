...
app.get('/api/high-spi', (req, res) => {
    try {
        const highSpiRecords = spiData.filter(record => record.spi > 5);
        res.json({
            count: highSpiRecords.length,
            records: highSpiRecords
        });
    } catch (error) {
        next(error);
    }
...
app.get('/api/low-spi/count', (req, res) => {
    try {
        const lowSpiCount = spiData.filter(record => record.spi < 5).length;
        res.json({
            count: lowSpiCount
        });
    } catch (error) {
...
app.get('/api/hindi-records', (req, res) => {
    try {
        const hindiRecords = spiData.filter(record => record.language === "Hindi");
        res.json({
...
app.get('/api/sindhi-records/a', (req, res) => {
    try {
        const sindhiRecordsA = spiData.filter(record => 
            record.language === "Sindhi" && 
            record.name.startsWith('A')
        );
        res.json({
            count: sindhiRecordsA.length,
            records: sindhiRecordsA
        });
    } catch (error) {
        next(error);
    }
